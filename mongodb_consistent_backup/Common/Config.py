...
class ConfigParser(BaseConfiguration):
    ...
    def makeParser(self):
        ...
        parser.add_argument("--ssl.enabled", dest="ssl.enabled", help="Use an SSL secured database connection (default: false)", default=False, type=bool, action="store_true")
        parser.add_argument("--ssl.insecure", dest="ssl.insecure", help="Do not validate the SSL certificate and hostname of the server (default: false)", default=True, type=bool, action="store_true")
        ...
        parser.add_argument("--ssl.client_cert_file", dest="ssl.client_cert_file", help="Path to Client SSL PEM file (for optional client ssl auth)", default=None, type=str)
